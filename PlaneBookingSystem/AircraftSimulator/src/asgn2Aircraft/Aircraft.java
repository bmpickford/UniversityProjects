...
public abstract class Aircraft {
	...
	static final int CHECK_AVAILABLE_FIRST = 1;
	static final int CHECK_AVAILABLE_BUSINESS = 2;
	static final int CHECK_AVAILABLE_PREMIUM = 3;
	...
	public Aircraft(String flightCode,int departureTime, int first, int business, int premium, int economy) throws AircraftException {
		checkAircraftExceptions(flightCode, departureTime, first, business, premium, economy);
	...
	public void cancelBooking(Passenger p,int cancellationTime) throws PassengerException, AircraftException {
		if (!hasPassenger(p)){
			throw new AircraftException("Flight does not contain passenger error");
		} else {
			decrementCancelledPassengers(p);
	...
	public boolean flightFull() {
		if ((numFirst == firstCapacity) && (numBusiness == businessCapacity) && (numPremium == premiumCapacity) && (numEconomy == economyCapacity)) {
			return true;
		}
		else {
			return false;
	...
	public void upgradeBookings() {
		for (int i = CHECK_AVAILABLE_FIRST; i <= CHECK_AVAILABLE_PREMIUM; i++){
			for (Passenger p: seats){
				//upgrade business to first
				if ((i == CHECK_AVAILABLE_FIRST) && (p.getPassID().contains("J")) && (numFirst < firstCapacity)) {
					upgradePassengerHelper(p);
					numBusiness--;
					numFirst++;
				//upgrade premium to business
				} else if ((i == CHECK_AVAILABLE_BUSINESS) && (p.getPassID().contains("P")) && (numBusiness < businessCapacity)) {
					upgradePassengerHelper(p);
					numPremium--;
					numBusiness++;
				//upgrade economy to premium
				} else if ((i == CHECK_AVAILABLE_PREMIUM) && (p.getPassID().contains("Y")) && (numPremium < premiumCapacity)){
					upgradePassengerHelper(p);
					numEconomy--;
					numPremium++;
				}
	...
	/**
	 * Helper method to upgrade passenger. Upgrades passenger and adjusts status. 
	 * @param p
	 */
	private void upgradePassengerHelper(Passenger p) {
		p.upgrade();
	...
	/**
	 * Helper method to test exceptions for constructor.
	 * 
	 * @param flightCode
	 * @param departureTime
	 * @param first
	 ...
	 */
	private void checkAircraftExceptions(String flightCode, int departureTime, int first, int business, int premium,
			int economy) throws AircraftException {
		if ((flightCode == null) || (departureTime <= 0) || (first < 0) || (business < 0) || (premium < 0) || (economy < 0)){
			throw new AircraftException("Aircraft input error");
		}
	}
	...
	/**
	 * Helper method to increment counts based on passenger type. 
	 * 
	 * @param p
	 */
	private void incrementConfirmedCounts(Passenger p) {
		if (p.getPassID().contains("F")){
			numFirst++; 
		} else if (p.getPassID().contains("J")){
			numBusiness++;
	...
	/**
	 * Helper method to decrease counts based on passenger type. 
	 * 
	 * @param p
	 */
	private void decrementCancelledPassengers(Passenger p) {
		if (p.getPassID().contains("F")){
			numFirst--; 
		} else if (p.getPassID().contains("J")){
			numBusiness--;
